...
func byte_calcu() {
	// &, |, ^, <<, >>
	// &^   按位置零   a &^ b 将b的二进制位中为1的位，对应到a的二进制中的位，并把a的这些位上的值置为0
}
...
func flow_control() {
	...
	case 0:
		// do nothing
	case 1: fallthrough (强制执行下一个case的逻辑)
	case 2:
		fmt.Print("a")
	default:
	}
...
// go的高并发协程机制
// 名词：系统线程，处理机，协程
// 基本流转：每个系统线程是买票口，处理机是幼儿园老师，后面跟着一队儿童
//  		没带钱的儿童会自己跑到队尾(当协程占用过多时间时，它会自己中断，插入队尾)
// 			某个儿童在等很慢的票出票时，老师会带着其他儿童去别的买票口买票，之前的儿童在拿到票之后，会跑到某个队伍的尾部
//			(当某个协程被系统中断，例如等待I/O时,处理机会移动到另一个系统线程继续跑后面的协程，中断恢复之后，插入到某个队尾)